...
var retrievedObject = localStorage.getItem('asisKnowledgeData');
asisObject = JSON.parse(retrievedObject);
reOrderSkills();
updateSkillChart();
console.log('asisobject: ', asisObject);
...
var skillChartData = {
  ...
  datasets: [{
    label: "To Be Knowledge",
    backgroundColor: "rgba(4, 153, 180,0.5)",
    borderColor: 'rgb(4, 153, 180)',
    pointBackgroundColor: 'rgb(4, 153, 180)',
...
function reOrderSkills (){
 ...
 for (i=0; i<asisObject.AsisKnowledgeID.length; i++){ 
      ...
      if (idCounter>=asIsKnowledgeCounter){asIsKnowledgeCounter=idCounter}; // update the biggest ID number
  }}
...
function saveData(){
  var tobeKnowledgeData = asisObject;
  
  //----------------Save the Object----------------------
  // Put the object into storage
  localStorage.setItem('tobeKnowledgeData', JSON.stringify(tobeKnowledgeData));
  
  // Retrieve the object from storage
  var retrievedObject = localStorage.getItem('tobeKnowledgeData');
...
function nextPage(){
  if(asisObject.AsisKnowledgeID.length !== 0){
    saveData();
  localStorage.setItem("whereWeAre", "passToBeAbilities"); 
